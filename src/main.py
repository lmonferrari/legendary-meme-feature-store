...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(parent_dir, artifacts_path, execution_id), exist_ok=True)

    features = create_feature_store()
    features.to_csv(
        os.path.join(parent_dir, "feature_store", "feature_store.csv"),
        sep=";",
        index=False,
    )
    ...
    model_path = os.path.join(parent_dir, artifacts_path, execution_id, "model.joblib")
    model.save_model(model_path)
    predict_path = os.path.join(
        parent_dir, artifacts_path, execution_id, "predictions.csv"
    )
    model.save_predicts(predict_path)

    run_info_path = os.path.join(
        parent_dir, artifacts_path, execution_id, "run_info.json"
    )
    model.save_info(
